...
const unsigned long AUTOPILOT_ON = (28 * 1000) + DELAY_MSEC;  // Amount of time after which the auto-stop functionality is enabled
...
void loop() {
  ...
  if (distance >= DELAY_DIST && distance < (DELAY_DIST + 2)) {
    stop();
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Group ");
