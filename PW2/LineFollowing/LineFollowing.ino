...
const uint8_t REV_UPPER_GEAR = 160;
...
const uint8_t IRT_L = 150;
const uint8_t IRT_R = 200;
...
void loop() {
  ...
  if (rightIRdetect && leftIRdetect) {  // Both IR sensors are seeing the black line
    stop();
  }
  if (is_moving) {
  ...
  delay(35);  // Delay for a small amount of time to smooth out operations
}
